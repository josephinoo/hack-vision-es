...
import { bothHandsDetected } from './utils/assignPlayers'
...
function Game({ names, onBack }) {
  ...
  const { ready, error, detection, recognizeBurst, resetStability } =
  ...
  const frozen = phase === PHASE.RESULT || phase === PHASE.CAPTURE
  ...
  const displayGesture1 =
    phase === PHASE.RESULT ? roundResult?.gestures?.player1 : detection.player1?.gesture
  const displayGesture2 =
    phase === PHASE.RESULT ? roundResult?.gestures?.player2 : detection.player2?.gesture
    ...
    <div className="flex min-h-full flex-col">
      ...
      <main className="flex flex-1 flex-col items-center justify-center gap-4 p-4 lg:flex-row lg:items-stretch lg:justify-center">
        ...
        <div className="relative flex flex-1 flex-col items-center justify-center">
          ...
          <Countdown
            visible={phase === PHASE.COUNTDOWN}
            label={countdownLabel}
            stabilityRatio={detection.stabilityRatio}
          />

          <ResultOverlay
            visible={phase === PHASE.RESULT}
